# Core of hk-nix, mirroring lefthook.nix's lib/config/default.nix: a single
# value that is BOTH the `nix flake check` derivation and the carrier of the
# `.shellHook` used to install hooks in a dev shell. Same config, run the same
...
let
  lib = pkgs.lib;
  inherit (import ./render-pkl.nix { inherit lib; }) renderHkPkl;

  configFile = pkgs.writeText "hk.pkl" (renderHkPkl {
    inherit schemaPath settings;
  });

  # CI half: copy the source into a sandbox, make a throwaway git repo with all
  # files staged, and run the hook in check-only mode so it fails (not fixes)
  # on findings. Linters are referenced by absolute store path from `settings`,
  # so they need not be on PATH here.
  check =
